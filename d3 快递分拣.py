...
# Exercise 2.9
"""
现要求按考试成绩将低将学员们分成5组，全存在一个新的大列表⾥, 5组分别是90-100, 80-89, 70-79,60-69, 0-59
最终的数据格式如下：
new_stu_list = [
 [
 ["Alex",100],
...
stu_list = [['李渊', 82], ['李世民', 7], ['侯君集', 5], ['李靖', 58], ['魏征', 41], ['房玄龄', 64],
            ['杜如晦', 65], ['柴绍', 94], ['程知节', 45], ['尉迟恭', 94], ['秦琼', 54], ['长孙无忌', 85],
            ['李存恭', 98], ['封德彝', 16], ['段志玄', 44], ['刘弘基', 18], ['徐世绩', 86], ['李治', 19],
            ['武则天', 39], ['太平公主', 57], ['韦后', 76], ['李隆基', 95], ['杨玉环', 33], ['王勃', 49],
            ['陈子昂', 91], ['卢照邻', 70], ['杨炯', 81], ['王之涣', 82], ['安禄山', 18], ['史思明', 9],
            ['张巡', 15], ['雷万春', 72], ['李白', 61], ['高力士', 58], ['杜甫', 27], ['白居易', 5],
            ['王维', 14], ['孟浩然', 32], ['杜牧', 95], ['李商隐', 34], ['郭子仪', 53], ['张易之', 39],
            ['张昌宗', 61], ['来俊臣', 8], ['杨国忠', 84], ['李林甫', 95], ['高适', 100], ['王昌龄', 40],
            ['孙思邈', 46], ['玄奘', 84], ['鉴真', 90], ['高骈', 85], ['狄仁杰', 62], ['黄巢', 79],
            ['王仙芝', 16], ['文成公主', 13], ['松赞干布', 47], ['薛涛', 79], ['鱼玄机', 16], ['贺知章', 20],
            ['李泌', 17], ['韩愈', 100], ['柳宗元', 88], ['上官婉儿 五代十国：朱温', 55], ['刘仁恭', 6],
            ['丁会', 26], ['李克用', 39], ['李存勖', 11], ['葛从周', 25], ['王建', 13], ['刘知远', 95],
            ['石敬瑭', 63], ['郭威', 28], ['柴荣', 50], ['孟昶', 17], ['荆浩', 84], ['刘彟', 18],
            ['张及之', 45], ['杜宇', 73], ['高季兴', 39], ['喻皓', 50], ['历真', 70], ['李茂贞', 6],
            ['朱友珪', 7], ['朱友贞', 11], ['刘守光', 2]]
...
for stu, mark in stu_list:
    if 90 <= mark <= 100:
        new_stu_list[0].append([stu, mark])
    if 80 <= mark < 90:
        new_stu_list[1].append([stu, mark])
    if 70 <= mark < 80:
        new_stu_list[2].append([stu, mark])
    if 60 <= mark < 70:
        new_stu_list[3].append([stu, mark])
    if 0 <= mark < 59:
        new_stu_list[4].append([stu, mark])

for each in new_stu_list:
    print(each)
print("我是帅气的分割线".center(100, "-"))

# Exercise 3.9
# 3.9.1 ⽣成⼀个包含100个key的字典，每个value的值不能⼀样
dic1 = {}
value_list = random.sample(range(1, 500), 100)
...
请把这个dict中key⼤于5的值value打印出来。
"""
dic2 = {'k0': 0, 'k1': 1, 'k2': 2, 'k3': 3, 'k4': 4, 'k5': 5, 'k6': 6, 'k7': 7, 'k8': 8, 'k9': 9}
for k, v in dic2.items():
    if v > 5:
        print(f"⼤于5的值value{k, v}")
print("我是帅气的分割线".center(100, "-"))

# 3.9.3  把题2中value是偶数的统⼀改成-1
for k, v in dic2.items():
    if v % 2 == 0:
        dic2[k] = -1
print(f"把题2中value是偶数的统⼀改成-1:{dic2}")
print("我是帅气的分割线".center(100, "-"))

# 3.9.4 公司职员档案查询字典

dic3 = {
    "张三": [23, 13888571722, "法外狂徒", "10000"],
    "罗翔": [24, 13888571723, "刑法专家", "20000"],
    "李四": [25, 13888571724, "吃瓜群众", "5000"],
    "王某": [26, 13888571725, "程序猿", "15000"]}  # 公司职员字典
count = 3  # 查询次数
while count:
    name = input("请输入您要查找的职员姓名：(如“张三”)")
    if name in dic3:
        print(f"姓名：{dic3[name][0]}  年龄：{dic3[name][1]}  职业：{dic3[name][2]}  收入：{dic3[name][3]}")
        break
    else:
        count -= 1
        print(f"不在档案内，请重新输入！\n您还有{count}次机会。")
...
out_dic = {}  # 输出分拣字典
for recipients, city in express_list:
    if city[:2] not in out_dic:
        out_dic[city[:2]] = [[recipients, city]]  # 若当前快递的城市不在字典，则创建该城市的关键字，并添加收件人信息
    else:
        out_dic[city[:2]].append([recipients, city])  # 若在字典，则在该城市关键字列表添加收件人信息
for name, city in out_dic.items():
    print(name, city)
